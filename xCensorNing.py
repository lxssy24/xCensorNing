...
def mosaic_lines(img_path):
    ...
    with Image.open(img_path) as image:
        ...
        for box in box_list:
            x, y, w, h = box
            while y <= box[1] + box[3]:
                draw.line([(x, y), (x + w, y)], fill="black", width=int(10 * 0.35))
                y += int(box[3] * 0.15)
        image.save(img_path)
    ...
    try:
        ...
        for filename in image_files:
            ...
            try:
                ...
                # 所有模式都清 EXIF
                # ----------------------
                remove_exif(str(output_img_path))
...
if __name__ == "__main__":
    iface = gr.Interface(
        ...
        inputs=[
            ...
            gr.Number(
                label="🧩 NEIGHBOR 值 (像素化强度)",
                value=NEIGHBOR, minimum=0.0001, maximum=0.1, step=0.0001,
                info="值越小格子越大。推荐 0.001 - 0.05"
            ),
            gr.Slider(
                label="🔘 椭圆扁度 (aspect)", minimum=0.3, maximum=1.0, step=0.05, value=0.6,
                info="控制椭圆的扁度，越小越扁。"
            ),
            gr.Slider(
                label="↕ 垂直偏移 (offset_y_ratio)", minimum=-0.3, maximum=0.3, step=0.01, value=-0.1,
                info="控制马赛克区域上下偏移，负值上移。"
            ),
            gr.Slider(
                label="📏 遮罩范围倍率 (scale_multiplier)", minimum=0.8, maximum=2.0, step=0.1, value=1.2,
                info="控制马赛克区域整体放大。"
            )
        ],
